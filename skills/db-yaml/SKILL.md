...
description: >-
  ...
  against cs_edw or another project database.
---
...
```bash
bin/db/offline -s kunde              # column list          (edelweiss-ui wrapper)
...
Connection details live in `~/.config/edw/db-profiles.yml` (mode 600), never in a
...
- **PHI.** `cs_edw` holds patient data. Aggregate and count freely; never copy
