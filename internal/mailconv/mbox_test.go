...
func TestMboxMessage(t *testing.T) {
	...
	got, err := MboxMessage(msgs[0], "tb-backup/gridfactor", "Inbox")
	if err != nil {
		t.Fatal(err)
	}
	if got.Source != "tb-backup/gridfactor" {
		t.Errorf("Source = %q, want tb-backup/gridfactor", got.Source)
	}
	if got.Folder != "Inbox" {
...
func TestSplitMboxDirIdempotent(t *testing.T) {
	root := t.TempDir()
	mb := filepath.Join(root, "tb-backup", "andriy@example.com", "Inbox")
