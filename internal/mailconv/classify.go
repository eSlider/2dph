...
//     через релей платформы (Ruby Rodriguez и т.п.);
...
// инфраструктура-отправители из реальных accept-списков #268 (eslider@ /
// wheregroup) + очевидные аналоги. Торговые компании (amazon/ebay/банки/
// телекомы) сюда НЕ входят — это company-класс (граница — OPEN, решение
// владельца на N-1.2/N-1.4). гдеgroup.com отсутствует намеренно:
// компания-контекст, не сервис (спец-исключение #268).
var networkServiceDomains = []string{
	"gitlab.com", "paypal.com", "paypal.de", "markets-platform.com",
...
// (gitlab@wheregroup.com, projeqtor@…) или как левый лейбл домена
// (trac.wheregroup.com, wiki.wheregroup.com, gitlab) = сервис.
...
// «WhereGroup Wartung», «Team Bit», «winSIM Neuigkeiten», «OGC Digest» —
...
func isPersonName(name string) bool {
	...
	// последний токен — не роль («WhereGroup Wartung», «Status-Update Workwise»).
...
// particles (см. isPersonName), иначе «Wizard of Oz»/«Celine … von Workwise»
// теряют класс person.
...
func isNameToken(tok string) bool {
	...
	if hasLower {
		return true // Title-case («Rothstein», «Hernández»)
	}
...
// «status-update», «WhereGroup Wartung», «Status-Update Workwise» → не имя.
