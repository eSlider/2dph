...
func seedTempBrain(t *testing.T, dbpath string) {
	...
	ids, err := AddLeafs(conn, []LeafInput{
		{Text: "first neuron a", Source: "mcp-agent", Root: "facts", Confidence: "confirmed", Type: "fact"},
		{Text: "second neuron b", Source: "mcp-agent", Root: "facts", Confidence: "confirmed", Type: "fact"},
		{Text: "third neuron c", Source: "mcp-agent", Root: "info", Confidence: "partial", Type: "reference"},
...
func TestQueryLeafsByFilters(t *testing.T) {
	...
	got, err = queryLeafs("", "", "mcp-agent", "", 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 3 {
		t.Fatalf("source=mcp-agent = %d leafs, want 3", len(got))
	}
	for _, lf := range got {
		if lf["source"] != "mcp-agent" {
			t.Fatalf("wrong source: %v", lf["source"])
		}
	}

	// combined filters
	got, err = queryLeafs("facts", "fact", "mcp-agent", "", 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 {
...
func TestSynapseHTTPServerEndToEnd(t *testing.T) {
	...
	// leafs by source (mcp-agent)
	code, body = get("/leafs?source=mcp-agent", "sekrit")
	if code != http.StatusOK || !strings.Contains(string(body), `"count":3`) {
		t.Fatalf("leafs source=mcp-agent code=%d body=%s", code, body)
	}
