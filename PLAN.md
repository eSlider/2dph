...
RAG over the operational Edelweiss/produktor/eSlider stack. Built like Sherlock
...
curasoft-detective method: **a fact needs ≥2 independent sources or it is
`(not confirmed)`.**
...
| D2 | skill merging | integrate skills **in this project** `skills/`; skip gitea / edelweiss-curasoft-depe ndent skills. |
| D3 | web search | import `web-search`, retire local `searxng-produktor`. Vendored here, no remote link. |
...
- `search.env` — real `EDW_SEARCH_URL/USER/PASS` from `~/.config/produktor/npm-bot.env`
...
- `~/.config/edw/../` — nothing else lives in the repo.
