...
| D32 | ANN вне liblbug (#201/#204) | Инкрементальный HNSW в `internal/brain/ann` на **coder/hnsw** (чистый Go, CC0; DataDog/hnswlib удалён, usearch = CGO-ад). Индекс `var/state/vector.ann` + append-only WAL (`upsert` = только новые leafs, без rebuild). `bin/brain/ann.go` (build/upsert/stats/api/search), wave-шаг `ann-upsert`. `queryVector`: ANN top-k → point-lookup метаданных → слияние с FTS (rank не тронут); fallback на скан при отсутствии/повреждении. Конфиг `vector.ann` (M=32 дефолт: при M=16 recall ~0.8, при 32 ~1.0; efS=400). A/B: baseline p50 ~31s → ANN p50 <1s, recall@5 vs baseline ≥ 0.95 (цифры в #204). |
...
`Leaf(embedding FLOAT[N])` — FTS on `text`. Векторный поиск: ANN HNSW вне
