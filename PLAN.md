...
| D32 | ANN вне liblbug (#201/#204) | Инкрементальный векторный индекс в `internal/brain/ann`: **IVF** (k-means + точный косинус по probed-ячейкам, чистый Go; `coder/hnsw`/HNSW отброшен — на 313k реальных достижимы 34% узлов, recall@5 0.16). Индекс `var/state/vector.ann` + append-only WAL (`upsert` = только новые leafs, без rebuild). `bin/brain/ann.go` (build/upsert/stats/api/search), wave-шаг `ann-upsert`. `queryVector`: индекс top-k → point-lookup метаданных → слияние с FTS (rank не тронут); fallback на скан при отсутствии/повреждении. Конфиг `vector.ann` (NList=2000, NProbe=128: recall@30 ≥ 0.93 на 313k, p50 ~100ms). A/B: baseline p50 ~31s → кандидат p50 ~0.33s, recall@5 vs baseline ≥ 0.95 (цифры в #204). |
