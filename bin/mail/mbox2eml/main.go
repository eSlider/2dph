// bin/mail/mbox2eml - split mbox mailboxes into one .eml per message.
//
//	./bin/mail/mbox2eml.go --in DIR --out var/mail/archive --dry-run
//
...
func main() {
	...
	flag.Parse()
	if in == "" {
		fmt.Fprintln(os.Stderr, "mbox2eml: --in DIR required")
		os.Exit(2)
	}
	total := 0
